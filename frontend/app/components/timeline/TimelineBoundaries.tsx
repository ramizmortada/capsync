    ...
    <>
      ...
      {editableSegments.length > 0 && (
        <div
          ...
          className={`absolute top-[20px] h-8 w-8 -ml-4 z-30 flex justify-center items-center group ${cursorMode !== 'resize' ? 'pointer-events-none' : ''}`}
        ...
        if (zoomLevel >= 15 && segment.words && segment.words.length > 0) {
            ...
            if (nextWord) {
              ...
              elements.push(
                <div 
                  ...
                  className={`absolute top-[20px] h-8 w-8 -ml-4 z-50 flex justify-center items-center group ${cursorMode !== 'resize' ? 'pointer-events-none' : ''}`} 
          ...
          if (index < editableSegments.length - 1) {
            ...
            elements.push(
              ...
              >
                <div className={`w-0.5 h-full transition-colors ${draggingBoundary === 'end' ? 'bg-emerald-400 w-1' : 'bg-orange-500/40 group-hover:bg-orange-400 group-hover:w-1'}`} />
              </div>
            );
          }
          ...
          if (isTouching) {
            ...
            elements.push(
              <div key={`cluster-${index}`} className={`absolute top-[20px] h-8 w-16 -ml-8 z-30 flex ${cursorMode !== 'resize' ? 'pointer-events-none' : ''}`} style={{ left: `${leftPercent}%` }}>
                <div 
