...
export function SubtitleHeader({
  ...
  return (
    <div className="border-b border-border bg-card flex flex-col shrink-0">
      {/* ROW 1: TABS */}
      <div className="px-4 py-2 border-b border-border/40 flex justify-center items-center">
        <div className="flex items-center gap-1 bg-background p-1 rounded-lg border border-border">
          <Button
            variant={activeTab === 'subtitles' ? 'secondary' : 'ghost'}
            size="sm"
            onClick={() => setActiveTab('subtitles')}
            className={`h-7 px-4 gap-1.5 text-xs font-medium transition-all ${
              activeTab === 'subtitles'
      ...
      {activeTab === 'subtitles' && (
        <div className="px-4 py-2 flex justify-between items-center min-h-[48px]">
          ...
          <div className="flex items-center gap-2">
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  size="icon"
                  className="h-8 w-8 bg-red-950/20 text-red-400 border-red-900/30 hover:bg-red-900/40 hover:text-red-300"
            ...
            {selectedIndexesCount === 0 && (
              <div className="flex items-center gap-0.5 bg-background border border-border rounded-md overflow-hidden h-8">
                <button 
                  onClick={() => handleOffsetSegments(-0.1)} 
                  className="hover:bg-muted h-full px-2 transition-colors text-muted-foreground hover:text-foreground outline-none focus:outline-none flex items-center"
                  title="Shift all subtitles 100ms earlier"
                >
                  <ChevronLeft className="w-3.5 h-3.5" />
                </button>
                <div className="flex items-center gap-1 px-1.5 text-[10px] uppercase font-bold text-muted-foreground/80 select-none border-x border-border/50 h-full">
        ...
        <div className="flex items-center gap-2">
          ...
          {onExportToImageEditor && selectedIndexesCount > 0 && (
